...
import com.ihub.www.config.CustomUserDetailsService;
...
import com.ihub.www.utility.Constants.IsAccountLinked;
...
public class UserResource {
	...
	public ResponseEntity<CommonApiResponse> registerUser(RegisterUserRequestDto request) {
		...
		user.setStatus(UserStatus.ACTIVATED.value());
	...
	public ResponseEntity<UserListResponseDto> fetchBankManagers() {
		...
		List<User> users = new ArrayList<>();
		users = this.userService.getUsersByRolesAndStatusAndBankIsNull(UserRole.ROLE_BANK.value(), UserStatus.ACTIVATED.value());
	...
	public ResponseEntity<UserListResponseDto> searchBankCustomers(String customerName) {
